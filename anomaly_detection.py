...
if param['short_cut'] and os.path.isfile(f'./dataset_prepared/{mtype}/N{bg}/{rseed}/train_dataset.pkl'):
    ...
    train_data, valid_data, test_data, train_audio_name, valid_audio_name, test_audio_name = anomaly_dataset.anomaly_dataset(machine_type=param['machine_type'], bg = param['bg'], rseed=param['rseed'], excludes = param['exclude'])

    train_dataset = []

    for iaudio in range(len(train_audio_name)):
        if anomaly_dataset.pair2status_info(train_audio_name[iaudio]) == 'noise':
    ...
    for iaudio in range(len(valid_audio_name)):
        if anomaly_dataset.pair2status_info(valid_audio_name[iaudio]) == 'noise':
            a_label = np.ones(2) / 2 * 1.0
        else:
            a_label = np.zeros(2)
            a_label[0] = 1.0
    ...
    for iaudio in range(len(test_audio_name)):
        if anomaly_dataset.pair2status_info(test_audio_name[iaudio]) == 'noise':
            a_label = np.ones(2) / 2 * 1.0
        else:
            a_label = np.zeros(2)
            a_label[0] = 1.0
